...
export default class LinearNumeric {
  constructor (options = {}) {
    this._computedIncrement = options.increment === undefined ? 100 : options.increment
  }
  
  get increment () {
    return this._computedIncrement
  }
  set increment (increment) {
    this._computedIncrement = increment
  }

  invoke (options) {
    return linearNumeric.bind(this)(options)
  }
}

function linearNumeric (options = {}) {
  const { increment = this.increment, only = incrementable } = options,
